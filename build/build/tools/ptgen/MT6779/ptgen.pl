...
my @partition_layout_cooked_emmc;
my @partition_layout_cooked_ufs;
my @BR_INDEX = ();
my %download_files;
...
my $YAML_SCAT_FILE = $RootPackage ne "yes" ?
                "$PRODUCT_OUT/$ArgList{PLATFORM}_Android_scatter.txt" :
                "$PRODUCT_OUT/$ArgList{PLATFORM}_Android_scatter_root.txt";
my $XML_SCAT_FILE = $RootPackage ne "yes" ?
                "$PRODUCT_OUT/$ArgList{PLATFORM}_Android_scatter.xml" :
                "$PRODUCT_OUT/$ArgList{PLATFORM}_Android_scatter_root.xml";
my $PGPT_FILE_EMMC = "$PRODUCT_OUT/PGPT_EMMC";
my $PGPT_FILE_UFS = "$PRODUCT_OUT/PGPT_UFS";
my $PGPT_FILE_LEGACY = "$PRODUCT_OUT/PGPT";
...
if ($ArgList{EMMC_SUPPORT} eq "yes")
{
    ...
    @partition_layout_cooked_emmc = @{&ProcessRawPartitionLayoutData("emmc", \@partition_layout_raw)};
}
if ($ArgList{UFS_BOOTING} eq "yes")
{
    ...
    @partition_layout_cooked_ufs = @{&ProcessRawPartitionLayoutData("ufs", \@partition_layout_raw)};
}

print "###################Final Data###################\n";
if ($ArgList{EMMC_SUPPORT} eq "yes") {
    print Dump(\@partition_layout_cooked_emmc);
}
if ($ArgList{UFS_BOOTING} eq "yes") {
    print Dump(\@partition_layout_cooked_ufs);
}
...
{
    if ($ArgList{UFS_BOOTING} eq "yes") {
        &GenPartSizeFile_iniFile($PART_SIZE_LOCATION, $PRODUCT_OUT, \@partition_layout_cooked_ufs);
    } else {
        &GenPartSizeFile_iniFile($PART_SIZE_LOCATION, $PRODUCT_OUT, \@partition_layout_cooked_emmc);
    }
}

if ($ArgList{EMMC_SUPPORT} eq "yes")
{
   &GenPGPTFile("emmc", \@partition_layout_cooked_emmc);
}
if ($ArgList{UFS_BOOTING} eq "yes")
{
   &GenPGPTFile("ufs", \@partition_layout_cooked_ufs);
}
...
=cut
    ...
    $ArgList{MTK_AUDIODSP_SUPPORT} = $ENV{MTK_AUDIODSP_SUPPORT};
    ...
    if ($ArgList{MTK_AB_OTA_UPDATER} eq "yes")
    {
        ...
        {
            $AlignPartList{protect1} = 8388608;
            $AlignPartList{persist} = 8388608;
            $AlignPartList{sec1} = 8388608;
        }
        else
        {
            $AlignPartList{protect2} = 8388608;
        }
    }
    else
    {
...
sub ProcessRawPartitionLayoutData
{
    ...
    for ($partition_idx = 0 ; $partition_idx < @partition_layout_process ; $partition_idx++)
    {
        foreach my $col_name_idx (keys %{$partition_layout_process[$partition_idx]})
        {
            ...
            {
                if ("eng" ~~ $partition_layout_process[$partition_idx]->{$col_name_idx}) {
                ...
                } elsif ($storage_type ~~ $partition_layout_process[$partition_idx]->{$col_name_idx}) {
                    $partition_layout_process[$partition_idx]->{$col_name_idx} = $partition_layout_process[$partition_idx]->{$col_name_idx}->{$storage_type};
                }
            }
        }
    }
    ...
    for ($partition_idx = 0 ; $partition_idx < @partition_layout_process ; $partition_idx++)
    {
        ...
        {
            if (($storage_type eq "emmc" && $ArgList{EMMC_SUPPORT_OTP} ne "yes") || ($storage_type eq "ufs" && $ArgList{UFS_OTP} ne "yes"))
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        ...
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^audio_dsp(_[ab])?$/i)
        {
            if ($ArgList{MTK_AUDIODSP_SUPPORT} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
...
{
    my $storage_type = shift @_;
    my @partition_layout_cooked = @{shift @_};
    my %Scatter_Info;
    my $iter;
    my %root_files = (
...
sub GenYAMLScatFile()
{
    ...
    my $iter;
    my $storage_type = shift @_;
    my $uc_storage_type = uc $storage_type;
    my $ScatterFileFH = shift @_;
    my %header_info = %{shift @_};
    my @partition_layout_cooked = @{shift @_};

    %Scatter_Info = &GenScatInfo($storage_type, \@partition_layout_cooked);

    my $Head2 = <<"__TEMPLATE";
############################################################################################################
#
#  $uc_storage_type Layout Setting
#
############################################################################################################
__TEMPLATE
    ...
    if ($combo_emmc_ufs eq "yes") {
        ...
        print $ScatterFileFH "${rootDashes}storage_type${colon}$uc_storage_type\n";
        print $ScatterFileFH "${rootSpaceSymbol}description${colon}\n";
        print $ScatterFileFH "${FirstDashes}general${colon}MTK_STORAGE_CFG\n";
        print $ScatterFileFH "${FirstSpaceSymbol}info${colon}\n";
    }

    if ($combo_emmc_ufs eq "yes") {
        print $ScatterFileFH "${SecondDashes}storage${colon}$uc_storage_type\n";
    } else {
        print $ScatterFileFH "${SecondSpaceSymbol}storage${colon}$uc_storage_type\n";
    }
    print $ScatterFileFH "${SecondSpaceSymbol}boot_channel${colon}$header_info{storage}{$uc_storage_type}{boot_channel}\n";
    print $ScatterFileFH "${SecondSpaceSymbol}block_size${colon}$header_info{storage}{$uc_storage_type}{block_size}\n";
...
sub GenXMLScatFile()
{
    ...
    if ($combo_emmc_ufs eq "yes") {
        print $ScatterFileFH "${FirstSpaceSymbol}<storage_type name=\"$uc_storage_type\">\n";
        print $ScatterFileFH "${FirstSpaceSymbol}  <general name=\"MTK_STORAGE_CFG\">\n";
        print $ScatterFileFH "${FirstSpaceSymbol}    <storage name=\"$uc_storage_type\">\n";
        print $ScatterFileFH "${FirstSpaceSymbol}      <boot_channel>$header_info{storage}{$uc_storage_type}{boot_channel}</boot_channel>\n";
        print $ScatterFileFH "${FirstSpaceSymbol}      <block_size>$header_info{storage}{$uc_storage_type}{block_size}</block_size>\n";
        print $ScatterFileFH "${FirstSpaceSymbol}    </storage>\n";
        print $ScatterFileFH "${FirstSpaceSymbol}  </general>\n";
        $FirstSpaceSymbol       = "    ";
    }

    foreach my $part (@partition_layout_cooked)
    {
        print $ScatterFileFH "${FirstSpaceSymbol}<partition_index name=\"SYS$Scatter_Info{$part->{Partition_Name}}{partition_index}\">\n";
        print $ScatterFileFH "${FirstSpaceSymbol}  <partition_name>$part->{Partition_Name}</partition_name>\n";
        print $ScatterFileFH "${FirstSpaceSymbol}  <file_name>$Scatter_Info{$part->{Partition_Name}}{file_name}</file_name>\n";
        print $ScatterFileFH "${FirstSpaceSymbol}  <is_download>$Scatter_Info{$part->{Partition_Name}}{is_download}</is_download>\n";
        print $ScatterFileFH "${FirstSpaceSymbol}  <type>$Scatter_Info{$part->{Partition_Name}}{type}</type>\n";
        print $ScatterFileFH "${FirstSpaceSymbol}  <linear_start_addr>$Scatter_Info{$part->{Partition_Name}}{linear_start_addr}</linear_start_addr>\n";
        print $ScatterFileFH "${FirstSpaceSymbol}  <physical_start_addr>$Scatter_Info{$part->{Partition_Name}}{physical_start_addr}</physical_start_addr>\n";
        print $ScatterFileFH "${FirstSpaceSymbol}  <partition_size>$Scatter_Info{$part->{Partition_Name}}{partition_size}</partition_size>\n";
        print $ScatterFileFH "${FirstSpaceSymbol}  <region>$Scatter_Info{$part->{Partition_Name}}{region}</region>\n";
        print $ScatterFileFH "${FirstSpaceSymbol}  <storage>$Scatter_Info{$part->{Partition_Name}}{storage}</storage>\n";
        print $ScatterFileFH "${FirstSpaceSymbol}  <boundary_check>$Scatter_Info{$part->{Partition_Name}}{boundary_check}</boundary_check>\n";
        print $ScatterFileFH "${FirstSpaceSymbol}  <is_reserved>$Scatter_Info{$part->{Partition_Name}}{is_reserved}</is_reserved>\n";
        print $ScatterFileFH "${FirstSpaceSymbol}  <operation_type>$Scatter_Info{$part->{Partition_Name}}{operation_type}</operation_type>\n";
        print $ScatterFileFH "${FirstSpaceSymbol}  <is_upgradable>$Scatter_Info{$part->{Partition_Name}}{is_upgradable}</is_upgradable>\n";
        print $ScatterFileFH "${FirstSpaceSymbol}  <empty_boot_needed>$Scatter_Info{$part->{Partition_Name}}{empty_boot_needed}</empty_boot_needed>\n";
        print $ScatterFileFH "${FirstSpaceSymbol}  <reserve>0x00</reserve>\n";
        print $ScatterFileFH "${FirstSpaceSymbol}</partition_index>\n";
    }

    if ($combo_emmc_ufs eq "yes") {
        $FirstSpaceSymbol = "  ";
        print $ScatterFileFH "${FirstSpaceSymbol}</storage_type>\n";
    }
}
...
sub GenHeaderInfo()
{
    my %header_info = (
        ...
        platform => $ArgList{PLATFORM},
        project => $ArgList{FULL_PROJECT}
    );

    if ($combo_emmc_ufs eq "yes") {
        $header_info{config_version} = "V2.1.0";
    } else {
        $header_info{config_version} = "V1.1.2";
    }

    $header_info{storage} = { EMMC => {boot_channel => "MSDC_0", block_size => sprintf("0x%x", 2 * 64 * 1024)},
                              UFS  => {boot_channel => "UFSHCI_0", block_size => sprintf("0x%x", 512 * 1024)},
                              NAND => {boot_channel => "NONE", block_size => sprintf("0x%x", $ArgList{PAGE_SIZE} * 64 * 1024)}
                            };
    return %header_info;
}
...
sub GenYAMLScatAll()
{
    my $ScatterFileFH = &open_for_rw($YAML_SCAT_FILE);
    my %header_info = &GenHeaderInfo();
...
__TEMPLATE
    ...
    print $ScatterFileFH "${FirstDashes}general${colon}$header_info{general}\n";
    print $ScatterFileFH "${FirstSpaceSymbol}info${colon}\n";
    print $ScatterFileFH "${SecondDashes}config_version${colon}$header_info{config_version}\n";
    print $ScatterFileFH "${SecondSpaceSymbol}platform${colon}$header_info{platform}\n";
    print $ScatterFileFH "${SecondSpaceSymbol}project${colon}$header_info{project}\n";
    if ($ArgList{EMMC_SUPPORT} eq "yes") {
        &GenYAMLScatFile("emmc", $ScatterFileFH, \%header_info, \@partition_layout_cooked_emmc);
    }
    if ($ArgList{UFS_BOOTING} eq "yes") {
        &GenYAMLScatFile("ufs", $ScatterFileFH, \%header_info, \@partition_layout_cooked_ufs);
    }
    close $ScatterFileFH;
}
...
sub GenXMLScatAll()
{
    my $ScatterFileFH = &open_for_rw($XML_SCAT_FILE);
    my %header_info = &GenHeaderInfo();
    my $storage_type = "";
    if ($combo_emmc_ufs ne "yes") {
        if ($ArgList{EMMC_SUPPORT} eq "yes") {
            $storage_type = "EMMC";
        } else {
            $storage_type = "UFS";
    ...
    print $ScatterFileFH "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n\n";
    print $ScatterFileFH "<root>\n";
    print $ScatterFileFH "  <general name=\"$header_info{general}\">\n";
    print $ScatterFileFH "    <config_version name=\"$header_info{config_version}\">\n";
    print $ScatterFileFH "      <platform>$header_info{platform}</platform>\n";
    print $ScatterFileFH "      <project>$header_info{project}</project>\n";
    if ($storage_type ne "") {
        print $ScatterFileFH "      <storage>$storage_type</storage>\n";
        print $ScatterFileFH "      <boot_channel>$header_info{storage}{$storage_type}{boot_channel}</boot_channel>\n";
        print $ScatterFileFH "      <block_size>$header_info{storage}{$storage_type}{block_size}</block_size>\n";
    }
    print $ScatterFileFH "    </config_version>\n";
    print $ScatterFileFH "  </general>\n";

    if ($ArgList{EMMC_SUPPORT} eq "yes") {
        &GenXMLScatFile("emmc", $ScatterFileFH, \%header_info, \@partition_layout_cooked_emmc);
    }
    if ($ArgList{UFS_BOOTING} eq "yes") {
        &GenXMLScatFile("ufs", $ScatterFileFH, \%header_info, \@partition_layout_cooked_ufs);
    }
    print $ScatterFileFH "</root>\n";
    close $ScatterFileFH;
}
...
sub GenPartSizeFile_iniFile
{
    ...
    my $part_size = shift @_;
    my $inifilepath = shift @_;
    my @partition_layout_cooked = @{shift @_};
...
sub GenPGPTFile
{
    ...
    if ($combo_emmc_ufs eq "yes") {
        if ($storage_type eq "emmc") {
            $out_file_name = $PGPT_FILE_EMMC;
        } else {
            $out_file_name = $PGPT_FILE_UFS;
        }
    } else {
        $out_file_name = $PGPT_FILE_LEGACY;
    }
    # Generate PGPT binary file -----------------------------------------------------
    print("Generate $out_file_name bin file\n");
    #create file
    open(FH,">$out_file_name")|| die "create $out_file_name file failed\n";
    if ($storage_type eq "emmc") {
        $LBA_size = 512;
