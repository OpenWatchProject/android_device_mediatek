...
=cut
    ...
    $ArgList{TARGET_COPY_OUT_ODM}        = $ENV{MTK_ODM_SUPPORT};
    ...
    %download_files = (
                       ...
                       logo       => "logo.bin",
                       odmdtbo    => "odmdtbo.img",
                       odmdtbo_a  => "odmdtbo.img",
                       odmdtbo_b  => "odmdtbo.img",
                       vendor     => "vendor.img",
                       vendor_a   => "vendor.img",
                       vendor_b   => "vendor.img",
                       system     => "system.img",
...
sub ProcessRawPartitionLayoutData
{
    ...
    #modify size for some part by common folder: such as :device/mediatek/common
    ...
    if ($board_config)
    {
        ...
        print "common  has no BoardConfig.mk \n";
    }
    ...
    if ($board_config)
    {
		...
		{
			$part->{Size_KB} = $1;
			print "by platform size $part_name = $1 KB\n";
 		}
                if ($ArgList{MTK_GMO_RAM_OPTIMIZE} eq "yes")
                {
                    if ($line =~ /\A\s*BOARD_MTK_GMO_${part_name}_SIZE_KB\s*:=\s*(\d+)/i || $line =~ /\A\s*BOARD_MTK_GMO_${part_name}_SIZE_KB\s*:=\s*(NA)/i)
        ...
        if ($flavor_board_config)
        {
            ...
            while (defined($line = <$flavor_board_config>))
            {
                if ($line =~ /\A\s*TARGET_COPY_OUT_VENDOR\s*:=\s*(.*)/i)
                {
                    ...
                    {
                        $vendor_partition_exists = "Y";
                        print "by flavor enable vendor partition\n";
                    }
                    else
                    {
                        $vendor_partition_exists = "N";
                        print "by flavor disable vendor partition\n";
                    }
                }
		foreach my $part (@partition_layout_process)
		{
			...
			{
				$part->{Size_KB} = $1;
				print "by flavor size $part_name = $1 KB\n";
			}
			if ($ArgList{MTK_GMO_RAM_OPTIMIZE} eq "yes")
			{
				if ($line =~ /\A\s*BOARD_MTK_GMO_${part_name}_SIZE_KB\s*:=\s*(\d+)/i || $line =~ /\A\s*BOARD_MTK_GMO_${part_name}_SIZE_KB\s*:=\s*(NA)/i)
				{
					$part->{Size_KB} = $1;
					print "by flavor size $part->{Partition_Name} (GMO) = $1 KB\n";
				}
			}
		}
            }
    ...
    for ($partition_idx = 0 ; $partition_idx < @partition_layout_process ; $partition_idx++)
    {
        ...
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^(tee1|tee2)$/i)
        {
            if ($ArgList{TRUSTONIC_TEE_ATF_SUPPORT} ne "yes" && $ArgList{TRUSTONIC_TEE_TEE_SUPPORT} ne "yes" ||
                $ArgList{MTK_AB_OTA_UPDATER} eq "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        }
	if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^(tee_a|tee_b)$/i)
        {
            if ($ArgList{TRUSTONIC_TEE_ATF_SUPPORT} ne "yes" && $ArgList{TRUSTONIC_TEE_TEE_SUPPORT} ne "yes" ||
                $ArgList{MTK_AB_OTA_UPDATER} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
	...
	if ($partition_layout_process[$partition_idx]->{Partition_Name} eq "efuse")
	{
        ...
        {
            if ($ArgList{MTK_A1_FEATURE} ne "yes" || $ArgList{MTK_AB_OTA_UPDATER} eq "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        ...
        {
            if ($vendor_partition_exists eq "N" || $ArgList{MTK_AB_OTA_UPDATER} eq "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        }
	if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^(vendor_a|vendor_b)$/i)
        {
            if ($vendor_partition_exists eq "N" || $ArgList{MTK_AB_OTA_UPDATER} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        ...
        {
            if ($ArgList{TARGET_COPY_OUT_ODM} ne "odm" || $ArgList{MTK_AB_OTA_UPDATER} eq "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        }
	if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^(odm_a|odm_b)$/i)
        {
            if ($ArgList{TARGET_COPY_OUT_ODM} ne "odm" || $ArgList{MTK_AB_OTA_UPDATER} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
	...
	if ($partition_layout_process[$partition_idx]->{Partition_Name} eq "lk")
        {
	...
	{
		if ($ArgList{MTK_LOADER_BACKUP} ne "yes" || $ArgList{MTK_AB_OTA_UPDATER} eq "yes")
		{
			splice @partition_layout_process, $partition_idx, 1;
			$partition_idx--;
		}
	}
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^(lk_a|lk_b)$/i)
        {
            if ($ArgList{MTK_AB_OTA_UPDATER} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        }
	if ($partition_layout_process[$partition_idx]->{Partition_Name} eq "boot")
        {
            if ($ArgList{MTK_AB_OTA_UPDATER} eq "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
        ...
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^(boot_a|boot_b)$/i)
        {
            if ($ArgList{MTK_AB_OTA_UPDATER} ne "yes")
            {
               splice @partition_layout_process, $partition_idx, 1;
	...
	if ($partition_layout_process[$partition_idx]->{Partition_Name} eq "odmdtbo")
	{
		if ($ArgList{DTBO_FEATURE} ne "yes" || $ArgList{MTK_AB_OTA_UPDATER} eq "yes")
		{
			splice @partition_layout_process, $partition_idx, 1;
			$partition_idx--;
		}
	}
	if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^(odmdtbo_a|odmdtbo_b)$/i)
	{
		if ($ArgList{DTBO_FEATURE} ne "yes" || $ArgList{MTK_AB_OTA_UPDATER} ne "yes")
		{
			splice @partition_layout_process, $partition_idx, 1;
			$partition_idx--;
		}
	}
	if ($partition_layout_process[$partition_idx]->{Partition_Name} eq "system")
        {
            if ($ArgList{MTK_AB_OTA_UPDATER} eq "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
        ...
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^(system_a|system_b)$/i)
        {
            if ($ArgList{MTK_AB_OTA_UPDATER} ne "yes")
            {
               splice @partition_layout_process, $partition_idx, 1;
...
sub GenYAMLScatFile()
{
    ...
    for ($iter = 0 ; $iter < @partition_layout_cooked ; $iter++)
    {
        ...
        if ($ArgList{EMMC_SUPPORT} eq "yes")
        {
            if (($ArgList{MTK_LOADER_BACKUP} eq "yes" || $ArgList{MTK_AB_OTA_UPDATER} eq "yes") && $part->{Partition_Name} eq "preloader")
            {
               $Scatter_Info{$part->{Partition_Name}}{region} = "EMMC_BOOT1_BOOT2";
            }
            else
            {
	...
	if ($part->{OTA_Update} eq "N")
        {
            if ($ArgList{MTK_LOADER_BACKUP} eq "yes" && ($part->{Partition_Name} eq "preloader" || $part->{Partition_Name} eq "lk" || $part->{Partition_Name} eq "lk2") || ($ArgList{MTK_AB_OTA_UPDATER} eq "yes" && $part->{Partition_Name} eq "preloader"))
            {
...
sub GenPartSizeFile_iniFile
{
    ...
    foreach my $part (@partition_layout_cooked)
    {
	    ...
	    my $part_name = $part->{Partition_Name};
            my $part_str1 = "_a";
            my $part_str2 = "_b";
            if ($part_name =~ /$part_str1/)
            {
                my $s = index($part_name, '_a');
                $part_name = substr($part_name, 0, $s);
            }
            if ($part_name =~ /$part_str2/)
            {
                next;
            }
...
sub GenPGPTFile
{
   ...
   {
       my $part = $partition_layout_cooked[$piter];
       if ((exists $SkipPartList{$part->{Partition_Name}}) == 0)
       {
