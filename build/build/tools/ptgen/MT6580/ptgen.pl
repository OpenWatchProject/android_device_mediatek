...
{
    #EMMC LK fastboot partition move to alps\mediatek\platform\mt6580\lk\include\platform\partition_fastboot.h
    &GenLK_PartitionC();
...
=cut
    ...
    $ArgList{GOOGLE_TRUSTY_TEE_SUPPORT}  = $ENV{MTK_GOOGLE_TRUSTY_SUPPORT};
...
sub ProcessRawPartitionLayoutData
{
    ...
    for ($partition_idx = 0 ; $partition_idx < @partition_layout_process ; $partition_idx++)
    {
        ...
        {
            if ($ArgList{TRUSTONIC_TEE_ATF_SUPPORT} ne "yes" && $ArgList{TRUSTONIC_TEE_TEE_SUPPORT} ne "yes" && $ArgList{GOOGLE_TRUSTY_TEE_SUPPORT} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
    ...
    for ($partition_idx = 0 ; $partition_idx < @partition_layout_process ; $partition_idx++)
    {
        if ($partition_layout_process[$partition_idx]->{Reserved} eq "N")
        {
            if ($partition_idx != 0 && $partition_layout_process[$partition_idx]->{Region} eq $partition_layout_process[$partition_idx - 1]->{Region})
            {
                ...
                {
                    # print "got aligned partition $raw_name\n";
