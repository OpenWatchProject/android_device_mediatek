...
$MTK_EMMC_SUPPORT_OTP= $ENV{MTK_EMMC_SUPPORT_OTP};
...
my $EMMC_COMPO	= "$CD_ALPS/device/mediatek/build/build/tools/ptgen/MT6580/mbr_addr.pl" ;
...
if($ENV{PTGEN_CHECKOUT} eq "yes")
{
  ...
  $COPY_PATH_KERNEL_PARTITION_H="kernel-3.4/arch/arm/mach-mt6580/$PROJECT/common";
  $COPY_PATH_PARTITION_DEFINE_H_NAME_K="kernel-3.4/arch/arm/mach-mt6580/$PROJECT/common";
  $COPY_PATH_PARTITION_DEFINE_C_NAME  ="kernel-3.4/arch/arm/mach-mt6580/$PROJECT/common";  
  $COPY_PATH_PMT_H_NAME_K="kernel-3.4/arch/arm/mach-mt6580/$PROJECT/common";  
  $COPY_PATH_COMBO_NAND_KERNELH="kernel-3.4/arch/arm/mach-mt6580/$PROJECT/common"; 
...
else
{
  ...
  $COPY_PATH_KERNEL_PARTITION_H="$CD_ALPS/kernel-3.4/arch/arm/mach-mt6580/$PROJECT/common";
  $COPY_PATH_PARTITION_DEFINE_H_NAME_K="$CD_ALPS/kernel-3.4/arch/arm/mach-mt6580/$PROJECT/common";
  $COPY_PATH_PARTITION_DEFINE_C_NAME  ="$CD_ALPS/kernel-3.4/arch/arm/mach-mt6580/$PROJECT/common";  
  $COPY_PATH_PMT_H_NAME_K="$CD_ALPS/kernel-3.4/arch/arm/mach-mt6580/$PROJECT/common";  
  $COPY_PATH_COMBO_NAND_KERNELH="$CD_ALPS/kernel-3.4/arch/arm/mach-mt6580/$PROJECT/common"; 
  ...
  {
  	print "fuck!!!!!!!!!$ENV{PTGEN_PROJECT_OUT}\n";
...
if($EMMC_SUPPORT eq "yes"){
	$SHEET_NAME = "emmc";
	if($MTK_EMMC_SUPPORT_OTP eq "yes"){
...
#]
	...
	#}
    open (CUSTOM_MEMORYDEVICE_H_NAME, "<$CUSTOM_MEMORYDEVICE_H_NAME") or &error_handler("Ptgen open CUSTOM_MEMORYDEVICE_H_NAME=$CUSTOM_MEMORYDEVICE_H_NAME fail!\n", __FILE__, __LINE__);
	my @lines;
	my $iter = 0;
	my $part_num;	
	my $MAX_address = 0;
...
# Fill MBR & EBR table -----------------------------------------------------
	for ($iter=0; $iter<@PARTITION_FIELD; $iter++) {   
		...
		if($PARTITION_FIELD[$iter] =~ /EBR(\d)/){
			...
			$BR[$1][0] = "$TMP_OUT_PATH/$PARTITION_FIELD[$iter]";
...
sub GenPartSizeFile
{
	...
	for($index=0;$index < $total_rows;$index++){
		...
		if($MTK_NAND_UBIFS_SUPPORT eq "yes" && $TYPE_FIELD[$index] eq "UBIFS/YAFFS2"){
			...
			my $INIFILE="$TMP_OUT_PATH/ubi_${part_name}.ini";
...
sub load_partition_info {
  ...
  else
  {
	  ...
	  my $SearchFile = 'partition_table_nand_MT6580.xls'; #Search File Name  
	  ...
	  {
	    #search folder list+{project}/partition_table_nand_MT6580.xls existence
...
print "PTGEN_ENV=$PTGEN_ENV\n";
  if(($PTGEN_ENV eq "ALL") || ($PTGEN_ENV eq "ANDROID") || ($PTGEN_ENV eq "PROJECT"))
  {
		...
		printf "check $SCAT_NAME_DIR/MT6580_Android_scatter.txt $COPY_SCATTER_BR_FILES_PATH\n";
		if (-e "$SCAT_NAME_DIR/MT6580_Android_scatter.txt") {
			printf "Copy $SCAT_NAME_DIR/MT6580_Android_scatter.txt $COPY_SCATTER_BR_FILES_PATH\n";
			copy_file("$SCAT_NAME_DIR/MT6580_Android_scatter.txt",$COPY_SCATTER_BR_FILES_PATH);
		}
