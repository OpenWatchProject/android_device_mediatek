...
{
    #EMMC LK fastboot partition move to alps\mediatek\platform\mt6755\lk\include\platform\partition_fastboot.h
    &GenLK_PartitionC();
...
=cut
    ...
    $ArgList{MTK_SIM_LOCK_POWER_ON_WRITE_PROTECT}	       = $ENV{MTK_SIM_LOCK_POWER_ON_WRITE_PROTECT};
    ...
    $ArgList{MTK_PRIVACY_PROTECTION_LOCK} = $ENV{MTK_PRIVACY_PROTECTION_LOCK};
...
sub ProcessRawPartitionLayoutData
{
    ...
    for ($partition_idx = 0 ; $partition_idx < @partition_layout_process ; $partition_idx++)
    {
        ...
        if ($partition_layout_process[$partition_idx]->{Partition_Name} eq "ppl")
        {
            if ($ArgList{MTK_PRIVACY_PROTECTION_LOCK} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        ...
        if ($partition_layout_process[$partition_idx]->{Reserved} eq "N")
        {
            if ($partition_idx != 0 && $partition_layout_process[$partition_idx]->{Region} eq $partition_layout_process[$partition_idx - 1]->{Region})
            {
                ...
                if (exists $AlignPartList{$raw_name})
                {
                    ...
                    if($partition_layout_process[$partition_idx]->{Partition_Name} eq "protect1" && $ArgList{MTK_SIM_LOCK_POWER_ON_WRITE_PROTECT} ne "yes")
                    {
