...
my $recovery_download_file_suffix;
...
=cut
    ...
    $ArgList{MTK_RECOVERY_RAMDISK_SPLIT} = $ENV{MTK_RECOVERY_RAMDISK_SPLIT};
    ...
    if ($ArgList{MTK_RECOVERY_RAMDISK_SPLIT} ne "yes")
    {
        $recovery_download_file_suffix = "";
    }
    else
    {
        $recovery_download_file_suffix = "-kernel";
    }
    #download files
    %download_files = (
...
sub ProcessRawPartitionLayoutData
{
    ...
    for ($partition_idx = 0 ; $partition_idx < @partition_layout_process ; $partition_idx++)
    {
        ...
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^dtbo$/i)
        {
        ...
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^dtbo([12])$/i)
        {
        ...
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^(recovery_ramdisk|recovery_vendor)$/i)
        {
            if ($ArgList{MTK_RECOVERY_RAMDISK_SPLIT} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
...
sub GenPartSizeFile_iniFile
{
    ...
    foreach my $part (@partition_layout_cooked)
    {
        ...
        if ($part->{Partition_Name} ne "${raw_name}_b")   # do not print for _b partition, its redundant setting
        {
            ...
            {
                $raw_name = GetPartBaseName($part->{Partition_Name});
            ...
            if (exists $mountPointMapList{$raw_name})
            {
                ...
                {
                    $part_size_content .= sprintf("MTK_BOARD_ROOT_EXTRA_SYMLINKS += /mnt/vendor/%s:%s\n", $mountPointMapList{$raw_name}, $mountPointMapList{$raw_name});
                }
            }
        }
