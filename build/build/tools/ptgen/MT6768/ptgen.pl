...
sub InitGlobalValue
{
    ...
    $ArgList{CUSTOM_MODEM}               = $ENV{CUSTOM_MODEM};
    $ArgList{MRDUMP_PARTITION_ENABLE}    = $ENV{MTK_MRDUMP_PARTITION_ENABLE};
    $ArgList{MD_CO_LOAD}                 = $ENV{MD_CO_LOAD};
...
sub ProcessRawPartitionLayoutData
{
        ...
        while (defined($line = <$board_config>))
        {
            ...
            if ($line =~ /\A\s*TARGET_COPY_OUT_PRODUCT\s*:=\s*(.*)/i)
            {
                ...
                else
                {
                    ...
                    print "by common disable product partition\n";
                }
            }
    ...
    for ($partition_idx = 0 ; $partition_idx < @partition_layout_process ; $partition_idx++)
    {
        ...
        if ($partition_layout_process[$partition_idx]->{Partition_Name} eq "md1img_1")
        {
            if ($ArgList{MD_CO_LOAD} ne "yes" || split(/ +/, $ArgList{CUSTOM_MODEM}) le 1)
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        ...
        if ($partition_layout_process[$partition_idx]->{Partition_Name} eq "mrdump")
        {
            if ($ArgList{MRDUMP_PARTITION_ENABLE} ne "yes" && not ($ArgList{MRDUMP_PARTITION_ENABLE} eq "debug" && $ArgList{TARGET_BUILD_VARIANT} ne "user"))
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        ...
        if ($partition_layout_process[$partition_idx]->{Reserved} eq "N")
        {
            if ($partition_idx != 0 && $partition_layout_process[$partition_idx]->{Region} eq $partition_layout_process[$partition_idx - 1]->{Region})
            {
                ...
                if (exists $AlignPartList{$raw_name})
                {
                    ...
                    if ($partition_layout_process[$partition_idx]->{Partition_Name} eq "protect2" && $ArgList{SIM_LOCK_POWER_ON_WRITE_PROTECT} ne "yes")
                    {
...
__TEMPLATE
    ...
    print $ScatterFileFH "${SecondDashes}config_version${colon}V1.1.8\n";
    ...
    print $ScatterFileFH "${SecondSpaceSymbol}modem_check${colon}false\n";
    print $ScatterFileFH "${SecondSpaceSymbol}modem${colon}NA\n";
    ...
    if ($ArgList{MD_CO_LOAD} ne "yes" && -e "vendor/mediatek/internal" && split(/ +/, $ArgList{CUSTOM_MODEM}) gt 1) {
        my @md_parts = ("md1img", "md1img_a", "md1img_b");
        my %Md_Map = (
                      0 => {md_type => "6177m", md_img => "md1img.img"},
                      1 => {md_type => "6177",  md_img => "md1img_1.img"},
                     );
        my $length = keys %Md_Map;
        my($file, $dir, $ext) = fileparse($SCAT_FILE, qr/\.[^.]*/);
        my $new_file;
        my $operation;

        foreach my $md_part (@md_parts) {
            if (exists $Scatter_Info{$md_part} && $Scatter_Info{$md_part}{is_download} eq "true") {
                $operation = "update";
                last;
            }
        }
        ...
        if ($operation eq "update") {
            for ($iter = 0; $iter < $length; $iter++) {
                $new_file = join '', $dir, $file, "_", $Md_Map{$iter}{md_type}, $ext;
                copy($SCAT_FILE, $new_file);
                @ARGV = $new_file;
                $^I = "";
                while (<>) {
                    s/^${SecondSpaceSymbol}modem_check${colon}false/${SecondSpaceSymbol}modem_check${colon}true/;
                    s/^${SecondSpaceSymbol}modem${colon}NA/${SecondSpaceSymbol}modem${colon}$Md_Map{$iter}{md_type}/;
                    s/^${FirstSpaceSymbol}file_name${colon}md1img.img/${FirstSpaceSymbol}file_name${colon}$Md_Map{$iter}{md_img}/;
                    print;
               }
            }
            unlink $SCAT_FILE;
    ...
    foreach my $part (@partition_layout_cooked)
    {
        ...
        if ($part->{Partition_Name} ne "${raw_name}_b")   # do not print for _b partition, its redundant setting
        {
            if ($raw_name eq "dtbo")
            {
                ...
                $part_size_content .= sprintf("BOARD_%sIMG_PARTITION_SIZE:=%d\n",uc($raw_name),$temp);
            }
...
sub GenPGPTFile
{
   ...
   {
       $LBA_size = 512;
       print FH pack("C17408",0x0);#pgpt current max size = 34 LBA = 34*512 = 17408 bytes for emmc
   }
   elsif ($ArgList{UFS_BOOTING} eq "yes")
   {
       $LBA_size = 4096;
       print FH pack("C24576",0x0);#pgpt current max size = 6 LBA =6*4096=24576 bytes for ufs
   }

   #Construct PMBR(LBA0)
   seek(FH,446,0); #PartitionRecord
   print FH pack("C12",0x00,0x00,0x00,0x00,0xEE,0x00,0x00,0x00,0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x00);
   seek(FH,510,0); #Signature
   print FH pack("C2",0x55,0xAA);

   #Construct Partition table header(LBA1)
   seek(FH,$LBA_size,0); #Signature
   print FH pack("C8",0x45,0x46,0x49,0x20,0x50,0x41,0x52,0x54);
   seek(FH,$LBA_size+0x8,0); #Revision
   print FH pack("C4",0x00,0x00,0x01,0x00);
   seek(FH,$LBA_size+0xC,0); #HeaderSize
   print FH pack("C4",0x5c,0x00,0x00,0x00);
   #Skip HeaderCRC32(4bytes) and Reserved(4bytes)
   seek(FH,$LBA_size+0x18,0); #MyLBA
   ...
   for ($piter = 0 ; $piter < @partition_layout_cooked ; $piter++)
   {
      ...
      {
         seek(FH,$LBA_size+0x20,0); #AlternateLBA
         #Put sgpt partition size here for lk update usage, new design->32KB for ufs&emmc   old-> 16.5KB for emmc
         print FH pack("Q",$part->{Size_KB}*1024/$LBA_size);
      }
      if($part->{Partition_Name} eq "pgpt")
      {
         seek(FH,$LBA_size+0x28,0); #FirstUsableLBA
         $FirstUsableLBA = $part->{Size_KB}*1024/$LBA_size;
         print FH pack("Q",$FirstUsableLBA);
      }
   }
   #Skip LastUsableLBA(8bytes) and DiskGUID(16bytes)
   seek(FH,$LBA_size+0x48,0); #PartitionEntryLBA
   print FH pack("C8",0x02,0x00,0x00,0x00,0x00,0x00,0x00,0x00);
   seek(FH,$LBA_size+0x50,0); #NumberOfPartitionEntries
   print FH pack("L",$MAX_GPT_PARTITION_NUM);
   seek(FH,$LBA_size+0x54,0); #SizeOfPartitionEntry
   print FH pack("L",$GPT_ENTRY_SIZE);

   #Construct Partition entries

   seek(FH,$LBA_size*2,0);
