...
ifdef PRELOADER_TARGET_PRODUCT
  ...
  ifeq ($(AB_OTA_UPDATER),true)
    ...
    PRELOADER_DIR := vendor/mediatek/proprietary/bootable/bootloader/preloader
    PRELOADER_UFS_HEADER_BLOCK := $(PRELOADER_DIR)/platform/$(MTK_PLATFORM_DIR)/headerblock_for_ota_fastboot_ufs
    PRELOADER_EMMC_HEADER_BLOCK := $(PRELOADER_DIR)/platform/$(MTK_PLATFORM_DIR)/headerblock_for_ota_fastboot_emmc
...
MTK_NOT_UPGRADABLE_CURRENT_IMAGES := $(sort $(MTK_NOT_UPGRADABLE_CURRENT_IMAGES))
$(foreach IMAGE_NAME,$(MTK_NOT_UPGRADABLE_CURRENT_IMAGES),\
  $(eval MTK_SPLIT_VENDOR_INSTALLED_RADIOIMAGE_TARGET += $(PRODUCT_OUT)/$(IMAGE_NAME))\
)
